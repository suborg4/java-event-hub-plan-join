...
public interface StatsRepository extends JpaRepository<EndpointHit, Long> {
    ...
    @Query("SELECT new ru.suborg.ehpj.stats.dto.ViewStats(h.app, h.uri, COUNT(DISTINCT h.ip)) " +
    ...
    @Query("SELECT new ru.suborg.ehpj.stats.dto.ViewStats(h.app, h.uri, COUNT(h.uri)) " +
            "FROM EndpointHit AS h " +
            "WHERE h.timestamp BETWEEN ?1 AND ?2 " +
            "GROUP BY h.app, h.uri " +
